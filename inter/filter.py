...
class FilterUI:
    ...
    def build_filter_ui(self):
        self.filters_on=self.app_state.get_filters_on()
        print("filtro en filterui",self.filters_on)
        if self.filters_on:
            ...
            self.data_table_container = ft.Column()
            self.ui_container = ft.Column(
                controls=[self.data_table_container],
                expand=True,
                scroll=True
            )
            data_table = self.bsd_interaction.consultar_trabajadores("1=1")
    ...
    def apply_filters(self, e):
        ...
        filtro = " AND ".join(filtros) if filtros else "1=1"
